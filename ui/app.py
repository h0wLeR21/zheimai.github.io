...
@app.route('/start-face-recognition') #######################################           Change Your path in this function
def start_face_recognition():
    try:
        # Windows: Open in a new terminal window
        if sys.platform == "win32":
            subprocess.Popen(["start", "cmd", "/k", "python", "D:/code/New_folder/face/facial_recognition.py"], shell=True)
    
        return jsonify({"message": "Face Recognition Started in a New Window!"})
    except Exception as e:
